...
- **重构日志与进度事件结构**：统一 cfb 输出给客户端的结构化事件和多语言日志，使烧录、擦除、存档等流程使用一致的消息结构；这是不保证旧客户端解析兼容性的协议升级。
- **Progress 事件新增 phase 字段**（erase/write/verify），客户端进度条不再需要解析文本。

### 修复

- **DTR 复位脉冲零间隔**（cartridge_link.rs）：Linux/termio 下控制线翻转即时完成，固件看不到复位脉冲导致串口不进协议模式；拉高保持 60ms 再释放（Windows 驱动延迟曾掩盖此问题，WSL/usbipd 实测暴露）。
- **3 处硬编码中文日志**：擦后重识别、RTC 日期/星期、停止/溢出标记改走 i18n（--lang en 下不再漏中文）。
- i18n 全语言补齐：ja/ko/fr/de/pt-BR 各 +75 key，es +5（8 语言全部 189/189）。

### 已知问题

- GBA SRAM bank 切换与存档回写修复在 SkyEmu 仓库（另见 directplay 分支）。
