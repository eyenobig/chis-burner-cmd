...
///
/// 上电后首包偶发丢失 / 接触抖动时 ID 或 CFI 可能空读：失败时复位后再试。
pub fn read_info(link: &mut CartridgeLink) -> FlashInfo {
    let mut best = FlashInfo {
        id: None,
        device_size: 0,
    ...
    // 快路径 1 次；仅在 ID/CFI 都像悬空时再短重试 1 次（避免无卡带时拖满 3 轮）。
    for attempt in 0..2 {
        if attempt > 0 {
        ...
        if flash_present(&flash) {
            return flash;
        }
        // 保留「稍好」的一次（有 ID 或有容量），供最终判定。
        if flash.id.is_some() && best.id.is_none() {
            best = flash;
        } else if flash.device_size > best.device_size {
            best = flash;
        }
    }
    best
...
/// flash 芯片是否在位：有效 Autoselect ID，或 CFI 给出合理容量（ID 偶发丢包时兜底）。
pub fn flash_present(flash: &FlashInfo) -> bool {
    let id_ok = flash
        .id
        .map_or(false, |id| !id.iter().all(|&b| b == 0xFF) && !id.iter().all(|&b| b == 0x00));
    let cfi_ok = ((1u64 << 20)..(1u64 << 28)).contains(&flash.device_size); // 1MiB..256MiB
    id_ok || cfi_ok
}
